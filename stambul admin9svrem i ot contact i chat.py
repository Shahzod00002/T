...
def create_connection():
    conn = sqlite3.connect(r'C:\Users\shahzod\fproject\bot.db')  # Укажите свой путь к базе данных
    return conn
...
def create_connection():
    conn = sqlite3.connect(r'bot1.db')
    return conn
...
def unfreeze_all_dishes(message):
    ...
    bot.send_message(message.chat.id, "Все замороженные блюда теперь недоступны.")
